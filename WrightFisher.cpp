...
double WrightFisher::eigenvalues(int k)
{
    double eig = k * (k + theta - 1) / 2.0;
    assert(eig > 0);
    return(eig);
}

double WrightFisher::Psi(double lambda, double par1, char subName)
{
    double drift;
    double scale;
    drift = 0.01;
    scale = 2.0;
    //// par1: rate, alpha, theta
    double psilambda;
    /// no subordination:
    if (subName == 'N') {
        psilambda = lambda;
    }
    /// poisson process with rate par1
    else if (subName == 'P') {
        psilambda = drift*lambda + par1 * (1 - exp(-lambda));
    }
    
    /// alpha-stable with par par1
    else if (subName == 'A') {
        psilambda = drift * lambda + pow(lambda, par1);
    }
    /// inverse-gausian with par theta;
    else if (subName == 'T') {
        psilambda = drift * lambda + sqrt(2 * lambda + pow(par1 - 1, 2) / 4.0) - sqrt(pow(par1 - 1, 2) / 4.0);
    }
    /// inverse-gamma with pars a=par1 and b=scale;
    else if (subName == 'G') {
        psilambda = drift * lambda + par1*log(1+lambda/scale);
    }
    else {
        /// no subordination
        psilambda = lambda;
...
double100 WrightFisher::Getd(
    ...
    double100 t, double par1, char subName)  /// Compute contributions to denom for DrawBridgePMF cases
{
  ...
  if (d[i] < 0.0) {
    ...
    for (int j = 0; j <= m; ++j) {
      ...
      d[i] += exp(Getlogakm<double100>(c1, c2) +
                  static_cast<double100>(- (WrightFisher::Psi(WrightFisher::eigenvalues( c1), par1, subName ) * t))) *
              Expected_Dirichlet;
...
double100 WrightFisher::Getd2(
    ...
    double100 t, double par1, char subName)  /// Compute contributions to denom for
...
double100 WrightFisher::GetdBridgeSame(
    vector<double100> &d, int i, double100 x,
    double100
        t, double par1, char subName)  /// Compute contributions to denom for DrawBridgePMFSame function
{
  if (i > static_cast<int>(d.size()) - 1) d.resize(i + 1, -1.0);
  if (d[i] < 0.0) {
    d[i] = 0.0;
    ...
    for (int j = 0; j <= m; ++j) {
      ...
      d[i] +=
          exp(Getlogakm<double100>(c1, c2) +
              static_cast<double100>(- (WrightFisher::Psi(WrightFisher::eigenvalues(c1), par1, subName) * t) + addon));
      assert(exp(Getlogakm<double100>(c1, c2)) > 0.0);
    }
  }
  assert(d[i] >= 0.0);
...
double100 WrightFisher::GetdBridgeInterior(
    vector<double100> &d, int i, double100 x, double100 z,
    double100 t, double par1, char subName)  /// Compute contributions to denom for DrawBridgePMFInterior
                  /// function
{
  if (i > static_cast<int>(d.size()) - 1) d.resize(i + 1, -1.0);
  if (d[i] < 0.0) {
...
double100 WrightFisher::UnconditionedDiffusionDensity(
    double100 x, double100 y, double100 t, double par1, char subName,
                       ...
                       /// where diffusion can be absorbed at any time
{
  ...
  while (!mSwitch) {
    if (thetaP.empty()) {
      ...
      density_inc = QmApprox(m, t, par1, subName, o) * addon;
    } else if (!(thetaP[0] > 0.0)) {
      double100 addon =
...
double100 WrightFisher::DiffusionDensityApproximationDenom(
    double100 x, double100 t, double par1, char subName,
    const Options
             ...
             /// density when diffusion conditioned on non-absorption
{
      ...
      (thetaDependent == 2))  /// Check whether we have a zero mutation entry
  {
    ...
    {
      if (!(x > 0.0) || !(x < 1.0)) {
        denom_inc = QmApprox(d, t, par1, subName, o) * static_cast<double100>(d);
      } else {
        denom_inc =
            QmApprox(d, t, par1, subName, o) * (1.0 - pow(x, static_cast<double100>(d)) -
    ...
    const Options &o)  /// Compute truncation to diffusion transition density
{
  ...
  double100 density = 0.0, density_inc,
            denom = DiffusionDensityApproximationDenom(x, t, par1, subName, o),
            threshold = max(1.0e-12 * denom, 1.0e-50);
  int mMode = static_cast<int>(floor(GriffithsParas(t).first)), m = mMode,
      mFlip = 1, mU = 0, mD = 0;
  bool mSwitch = false, mUpSwitch = false, mDownSwitch = false;
  /// do we need to insert new swf eigenvalues here? ie what does m*(m-1) represent
  while (!mSwitch) {
    if (thetaP.empty()) {
      if (!(x > 0.0)) {
        density_inc = QmApprox(m, t, par1, subName, o) * static_cast<double100>(m * (m - 1)) *
    ...
    } else if (!(thetaP[0] > 0.0))  //|| !( thetaP[1] > 0.0 ) )
    {
      ...
      if ( !( thetaP[1] > 0.0 ) )
      {
        ...
        /// do we need to alter theta + m - 2? do we need to apply Psi here?
      if (!(x > 0.0)) {
        density_inc = QmApprox(m, t, par1, subName, o) *
                      static_cast<double100>(WrightFisher::eigenvalues(m)) *
...
double100 WrightFisher::QmApprox(
    int m, double100 t, double par1, char subName, 
    const Options &o)  /// Compute an approximation to q_m(t)
{
           ...
           qm > 1.0)  /// If increments are big enough, keep going
    {
      ...
      qm += pow(-1.0, mkIndex - m) *
            exp(Getlogakm<double100>(mkIndex, m) +
                static_cast<double100>(-(WrightFisher::eigenvalues(mkIndex)) *
                                       (t) ));
      mkIndex++;
  ...
  /// NATHAN: adjust for subordination
  qm = qm * exp(-t * WrightFisher::Psi(WrightFisher::eigenvalues(m), par1, subName));
...
double100 WrightFisher::UnconditionedBridgeDensity(
    double100 x, double100 z, double100 y, double100 s, double100 t, double par1, char subName,
    const Options
             ...
             /// diffusion bridge when absorption can happen at any time
{
         ...
         0.0)  /// As long as increments are positive, keep computing
  {
    if (!(z > 0.0))  /// z = 0
    {
      denom_inc = QmApprox(d, t, par1, subName, o) * pow(1.0 - x, static_cast<double100>(d));
    } else  /// z = 1
    {
      denom_inc = QmApprox(d, t, par1, subName, o) * pow(x, static_cast<double100>(d));
    }
  ...
  if ((!(y > 0.0) && ((thetaP.empty() || !(thetaP[0] > 0.0)))) ||
    ...
    double100 density = 0.0,
              threshold =
                  max(exp(log(max(1.0e-300, QmApprox(mMode, s, par1, subName, o))) +
                          log(max(1.0e-300, QmApprox(kMode, t - s, par1, subName, o))) +
                          constcontr - log(eC)) *
                          1.0e-20,
                      1.0e-50);
    ...
    while (!mSwitch) {
      ...
      while (!kSwitch) {
        ...
        if (!(y > 0.0) && ((thetaP.empty() || !(thetaP[0] > 0.0)))) {
          num_inc =
              exp(log(max(1.0e-300, QmApprox(m, s, par1, subName, o))) +
                  log(max(1.0e-300, QmApprox(k, t - s, par1, subName, o))) +
    ...
    vector<int> modeGuess = mklModeFinder(
        x, z, s, t, par1, subName, o);  /// Compute mode over (m,k,l) to use as start points
    ...
    while (!mSwitch) {
      double100 qm = max(1.0e-300, QmApprox(m, s, par1, subName, o));
      ...
      while (!kSwitch) {
        double100 qk = max(1.0e-300, QmApprox(k, t - s, par1, subName, o));
        if (k != kMode) {
          if (kU > kD) {
                       ...
                       /// transition density of the diffusion bridge
{
  ...
  while (inc > 0.0)  /// As long as increments are positive, keep computing
  {
    if ((x > 0.0) && (x < 1.0) && (z > 0.0) && (z < 1.0))  /// x,z in (0,1)
    {
      ...
      inc = QmApprox(d, t, par1, subName, o) * betabin;
    } else if ((z > 0.0) && (z < 1.0) &&
               (!(x > 0.0) || !(x < 1.0)))  /// x in {0,1}, z in (0,1)
    {
      ...
      inc = QmApprox(d, t, par1, subName, o) * pdf(BETAZ, z);
    } else if ((x > 0.0) && (x < 1.0) &&
               (!(z > 0.0) || !(z < 1.0)))  /// x in (0,1), z in {0,1}
    {
      ...
      inc = QmApprox(d, t, par1, subName, o) * xcon *
    ...
    } else if (!(x < z) && !(x > z))  /// x,z in {0,1} and x=z
    {
      double100 para1 = (!(x > 0.0) ? thetaP[0]
                ...
                para2 = (!(x > 0.0) ? static_cast<double100>(thetaP[1] + d)
                                    : thetaP[1]);
      inc = QmApprox(d, t, par1, subName, o) / boost::math::beta<double100>(para1, para2);
    } else  /// x,z in {0,1} and x!=z
    {
      return QmApprox(0, t, par1, subName, o) /
    ...
    const Options &o)  /// Compute bridge density when x,z in (0,1) (any theta!)
{
  ...
  vector<int> modeGuess = mkljModeFinder(
      x, z, s, t, par1, subName, o);  /// Compute mode over (m,k,l,j) to use as start points
  ...
  double100 qmmode = max(1.0e-300, QmApprox(mMode, s, par1, subName, o)),
            qkmode = max(1.0e-300, QmApprox(kMode, t - s, par1, subName, o));
  ...
  double100 density = 0.0, eC = BridgeDenom(x, z, y, s, t, par1, subName, o),
            threshold =
  ...
  /// Allows to avoid calling beta functions, and thus faster
  while (!mSwitch) {
    double100 qm = QmApprox(m, s, par1, subName, o);
    if (m != mMode)  /// Increment m contributions accordingly
    {
      if (mU > mD) {
        mContr_U +=
    ...
    while (!kSwitch) {
      double100 qk = QmApprox(k, t - s, par1, subName, o);
      if (k != kMode) {
        if (kU > kD) {
          kContr_U +=
        ...
        &o)  /// Compute bridge density when x in {0,1},z in (0,1) (any theta!)
{
  ...
  vector<int> modeGuess = mkjModeFinder(
      x, z, s, t, par1, subName, o);  /// Compute mode over (m,k,j) to use as start points
  int mMode = modeGuess[0], kMode = modeGuess[1],
        ...
        &o)  /// Compute bridge density when x,z in {0,1}, x=z (any theta!)
{
  ...
  vector<int> modeGuess = mkModeFinder(x, z, s, t, par1, subName, o);  /// Find mode over (m,k)
                       ...
                       /// computeDensity to invoke
{
  ...
  if ((x > 0.0) && (x < 1.0) && (z > 0.0) && (z < 1.0))  /// x,z in (0,1)
  {
    return ComputeDensity1(x, z, y, s, t, par1, subName,o);
  } else if ((x > 0.0) && (x < 1.0))  /// x in (0,1), z in {0,1}
  {
    return ComputeDensity2(z, x, y, t - s, t, par1, subName,
  ...
  {
    return ComputeDensity2(x, z, y, s, t, par1, subName, o);
  } else if (x == z)  /// x,z in {0,1}, x=z
  {
    return ComputeDensity3(x, z, y, s, t, par1, subName, o);
  } else  /// x,z in {0,1}, x!=z
  {
    return ComputeDensity4(x, z, y, s, t, par1, subName, o);
  }
}
...
pair<int, int> WrightFisher::DrawAncestralProcess(
    double100 t, double par1, char subName, const Options &o,
    boost::random::mt19937 &gen)  /// Draws from the law of the Ancestral
                                  /// Process using upper and lower sums
    /// starting from an entrance boundary at infinity(?)
{
  assert(t > 0.0);
            ...
            dL[m] < 0.0))  /// Checking if upper and lower sums converging
    {
      ...
      double100 newcoefficientU =
          ...
          exp(Getlogakm<double100>(n, m) +
              static_cast<double100>(-WrightFisher::Psi(WrightFisher::eigenvalues(n), par1, subName) * t));
      double100 newcoefficientL =
          (((n + 1 - m) % 2 != 0) ? -1.0 : 1.0) *
          exp(Getlogakm<double100>(n + 1, m) +
              static_cast<double100>(- WrightFisher::Psi(WrightFisher::eigenvalues(n+1), par1, subName) * t ));
        ...
        dU[k] = dL[k] +
                (((n_used[k] - k) % 2 != 0) ? -1.0 : 1.0) *
                    exp(Getlogakm<double100>(n_used[k], k) +
                        static_cast<double100>(
                            - WrightFisher::Psi(WrightFisher::eigenvalues(n_used[k]), par1, subName) * t ));
                      ...
                      /// from normalising constant (because m can only be >= 2)
      {
        double100 newcoefficientUq1 =
                      ...
                      exp(Getlogakm<double100>(n1, 1) +
                          static_cast<double100>(- WrightFisher::Psi((WrightFisher:: eigenvalues (n1)), par1, subName) * t )),
                  newcoefficientLq1 =
                      ((n1 % 2 != 0) ? -1.0 : 1.0) *
                      exp(Getlogakm<double100>(n1 + 1, 1) +
                          static_cast<double100>(
                              - WrightFisher::Psi((WrightFisher::eigenvalues(int (n1+1))),par1, subName) * t ));
    ...
    while (!decision_on_m_made)  /// Refine upper and lower bounds
    {
      ...
      for (int k = max(mmode, threshold) - mdown;
        ...
        if (thetaP.empty() && ((n_used[k] + 1) > q1index)) {
          double100 newcoefficientUq1 =
                        ...
                        exp(Getlogakm<double100>(n_used[k], 1) +
                            static_cast<double100>( - WrightFisher::Psi(WrightFisher::eigenvalues(n_used[k]), par1, subName) * t)),
                    newcoefficientLq1 =
                        (((n_used[k]) % 2 != 0) ? -1.0 : 1.0) *
                        exp(Getlogakm<double100>(n_used[k] + 1, 1) +
                            static_cast<double100>(
                                - WrightFisher::Psi(WrightFisher::eigenvalues(int (n_used[k]+1) ), par1, subName) * t));
          q1U = q1L + newcoefficientUq1;
          q1L = q1U + newcoefficientLq1;
        }
        ...
        if ((v_used[k] + 1) > eCindex_computed) {
          eCL += eCnew;
          eCnew =
              exp(static_cast<double100>(- WrightFisher::Psi(WrightFisher::eigenvalues(v_used[k] + 2) , par1, subName)* t ) +
                  log(static_cast<double100>(2 * (v_used[k] + 2) + theta - 1)));/// NATHAN: no t therefore no change
          eCU = eCL + (eCnew /
                       (1.0 - consForD * exp(-static_cast<double100>(
                                                 v_used[k] + 2 + theta / 2.0) * /// NATHAN: depends on t but not of the eigenvalue form
                                             t)));
          eCindex_computed += 1;
        }
...
pair<pair<int, int>, int> WrightFisher::DrawAncestralProcessConditionalInterior(
    double100 t, double100 x, double par1, char subName, const Options &o,
    boost::random::mt19937
        &gen)  /// Draws from the law of the Ancestral Process when conditioning
               /// the diffusion on non-absorption but started from inside (0,1)
{
  ...
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU = Getd2(eCvec, 0, x, t, par1, subName);
  map<int, double100> eAL, eAU, dL, dU;
  ...
  while (!ml_found) {
            ...
            dL[m] < 0.0))  /// Checking if upper and lower sums converging
    {
      ...
      if (2 * (v + 1) > eCindex_computed)  /// Computing denominator
      {
        ...
        eCL = eCU - Getd2(eCvec, 2 * v + 1, x, t, par1, subName);
        eCU = eCL + Getd2(eCvec, 2 * v + 2, x, t, par1, subName);
        eCindex_computed += 2;
      }
      ...
      while (!decision_on_ml_made)  /// Refine upper and lower bounds
      {
        ...
        for (int k = max(mmode, threshold) - mdown;
          ...
          if (2 * v_used[k] + 2 > eCindex_computed) {
            ...
            eCL = eCU - Getd2(eCvec, 2 * v_used[k] + 1, x, t, par1, subName);
            eCU = eCL + Getd2(eCvec, 2 * v_used[k] + 2, x, t, par1, subName);
            eCindex_computed += 2;
          }
...
pair<double100, int> WrightFisher::DrawEndpoint(
    double100 x, double100 t1, double100 t2, double par1, char subName, const Options &o,
                                  ...
                                  /// diffusion conditioned on non-absorption
{
  ...
  if (thetaP.empty())  /// No mutation - condition on avoiding either boundary
  {
          ...
          0.0))  /// Diffusion conditioned on non-absorption but started from 0
    {
      ...
      {
        m_and_coeffcount = DrawAncestralProcessConditionalZero(t2 - t1, par1, subName, o, gen);/// NATHAN: what we want to edit
                            ...
                            /// started from 1
    {
      ...
      {
        m_and_coeffcount = DrawAncestralProcessConditionalZero(t2 - t1, par1, subName, o, gen);
        m = m_and_coeffcount.first;
        coeffcount = m_and_coeffcount.second;
      }
            ...
            /// interior (0,1)
    {
      ...
      } else  /// Otherwise use alternating series technique
      {
        pair<pair<int, int>, int> storer =
            DrawAncestralProcessConditionalInterior(t2 - t1, x, par1, subName, o, gen);
                                    ...
                                    /// avoiding one boundary only
  {
                              ...
                              /// started from 0 by symmetry
    {
      ...
      if (t2 - t1 <=
          o.g1984threshold)  /// Time increment smaller than threshold, use
                             /// Gaussian approximations
      {
        m = DrawSizebiasedAncestralProcess(1, t2 - t1, gen);/// NATHAN: want to edit as well
        coeffcount = -1;
      } else  /// Otherwise use alternating series technique
      {
        m_and_coeffcount = DrawAncestralProcessConditionalZero(t2 - t1, par1, subName, o, gen);
                        ...
                        /// in interior (0,1] by symmetry
    {
      ...
      {
        pair<pair<int, int>, int> storer =
            DrawAncestralProcessConditionalInterior(t2 - t1, 1.0 - x, par1, subName, o, gen);///NATHAN: edit?
        m = (storer.first).first;
        l = (storer.first).second;
        coeffcount = storer.second;
      }
    ...
    } else  /// Diffusion conditioned on avoiding 0 but started from x in (0,1]
    {
      ...
      {
        pair<pair<int, int>, int> storer =
            DrawAncestralProcessConditionalInterior(t2 - t1, x, par1, subName, o, gen);///NATHAN: edit?
        m = (storer.first).first;
        l = (storer.first).second;
        coeffcount = storer.second;
      }
          ...
          /// diffusion
  {
    ...
    {
      m_and_coeffcount = DrawAncestralProcess(t2 - t1, par1, subName, o, gen);///NATHAN: MAIN CALL TO EDIT!
      m = m_and_coeffcount.first;
      coeffcount = m_and_coeffcount.second;
    }
...
pair<double100, int> WrightFisher::DrawUnconditionedDiffusion(
    double100 x, double100 t, double par1, char subName, const Options &o,
    boost::random::mt19937 &
        gen)  /// Draws from the law of an unconditioned Wright-Fisher diffusion
{/// NATHAN: THIS IS OUR CASE BC WE WANT +ve MUTATION RATES
  assert((x >= 0.0) && (x <= 1.0) && (t > 0.0));
  if (!(x > 0.0) &&
      (thetaP.empty() ||
  ...
  } else  /// Otherwise we first draw M ~ {q_m}
  {
    ...
    if (t <= o.g1984threshold) {
      ...
      pair<int, int> temp = DrawAncestralProcess(t, par1, subName, o, gen);///NATHAN: EDIT THIS FUNCTION
      m = temp.first;
...
vector<vector<double100>> WrightFisher::NonNeutralDraw(
    double100 x, double100 t1, double100 t2, double par1, char subName, bool Absorption, const Options &o,
                                  ...
                                  /// diffusion started from x at time t1
{
  ...
  while (!accept)  /// Until all skeleton points get accepted, keep going
  {
    ...
    for (vector<double100>::iterator itt = times.begin(), itm = marks.begin();
      ...
      {
        if (Absorption) {
          path.push_back(DrawUnconditionedDiffusion(x, t2 - t1, par1, subName, o, gen)
                             .first);  /// Generate endpoint
        } else {
          path.push_back(
              DrawEndpoint(x, t1, t2, par1, subName, o, gen).first);  /// Generate endpoint
        }
      ...
      } else  /// kappa > 0 - generate skeleton points and endpoint
      {
        if (itt == times.begin()) {
          if (Absorption) {
            path.push_back(
                DrawUnconditionedDiffusion(x, (*itt) - t1, par1, subName, o, gen)
                    .first);  /// Generate skeleton points sequentially
          } else {
            path.push_back(
                DrawEndpoint(x, t1, *itt, par1, subName, o, gen)
                    .first);  /// Generate skeleton points sequentially
          }
                                ...
                                /// correct sequential sampling
        {
          if (Absorption) {
            path.push_back(DrawUnconditionedDiffusion(
                               path.back(), (*itt) - *(itt - 1), par1, subName, o, gen)
                               .first);
          } else {
            path.push_back(
                DrawEndpoint(path.back(), *(itt - 1), *itt, par1, subName, o, gen).first);
          }
...
{
    vector<vector<double100>> ptr = NonNeutralDraw(x, t1, t2, par1, subName, Absorption, o, gen);
...
vector<int> WrightFisher::DrawBridgePMFUnconditional(
    double100 x, double100 z, double100 s, double100 t, double par1, char subName, const Options &o,
    boost::random::mt19937
               ...
               /// with time increments being large enough
{
  ...
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(GetdBridgeUnconditional(eCvec, 0, x, z, t, par1, subName));
  vector<int> v_used;
  double100 Amkl;
  ...
  while (!mkl_found) {
    ...
    while (2 * v < Fmkl || eAU[n] > 1.0 || eBU[n] > 1.0) {
      ...
      if (2 * v + 2 > eCindex_computed) {
        assert(2 * v == eCindex_computed);
        eCL = eCU - GetdBridgeUnconditional(eCvec, 2 * v + 1, x, z, t, par1, subName);
        eCU = eCL + GetdBridgeUnconditional(eCvec, 2 * v + 2, x, z, t, par1, subName);
        eCindex_computed += 2;
      }
          ...
          eCL == eCU)  /// ...then we have lost precision before reaching Fmkj.
      {
        ...
        if (eAL[n] < 0.0 || eCL < 0.0) {
          cerr << "Numerical error detected: (m,k) = " << m << "," << k
               ...
               << "," << t << ") ..." << endl;
          return DrawBridgePMFUnconditionalApprox(x, z, s, t, par1, subName, o, gen);
        }
      ...
      while (!decision_on_mkl_made)  /// Refine upper and lower bounds
      {
        ...
        for (int i = 0; i <= n; ++i) {
          ...
          if (2 * v_used[i] + 2 > eCindex_computed) {
            ...
            eCL = eCU -
                  GetdBridgeUnconditional(eCvec, 2 * v_used[i] + 1, x, z, t, par1, subName);
            eCU = eCL +
                  GetdBridgeUnconditional(eCvec, 2 * v_used[i] + 2, x, z, t, par1, subName);
            eCindex_computed += 2;
          }
               ...
               /// with one time increment falling below the threshold
{
  ...
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(GetdBridgeUnconditional(eCvec, 0, x, z, t, par1, subName)), qApprox = 0.0,
            runningMax = 1.0e-300;
  vector<int> v_used;
  double100 Amkl;
  int n = -1, Fmkl = 0, eCindex_computed = 0;
               ...
               /// with both time increments falling below the threshold
{
  ...
  while (max(eCOldInc, eCInc) > 0.0 || (!(eC > 0.0))) {
    ...
    eCInc = exp(log(QmApprox(d, t, par1, subName, o)) + xcont);
    eC += eCInc;
  ...
  vector<int> modeGuess = mklModeFinder(
      x, z, s, t, par1, subName, o);  /// Get a guess to location of mode over (m,k,l)
  int mMode = modeGuess[0], kMode = modeGuess[1], lMode = modeGuess[2];
...
vector<int> WrightFisher::DrawBridgePMFSameMutation(
    double100 x, double100 s, double100 t, double par1, char subName, const Options &o,
    boost::random::mt19937
               ...
               /// same boundary point and time increments are large enough
{
  ...
  double100 u = U01(gen), currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(GetdBridgeSame(eCvec, 0, x, t, par1, subName));
  vector<int> v_used;
  ...
  while (!mk_found) {
    ...
    while (2 * v < Fmkj || eAU[n] > 1.0 || eBU[n] > 1.0) {
      ...
      if (2 * v + 2 > eCindex_computed) {
        assert(2 * v == eCindex_computed);
        eCL = eCU - GetdBridgeSame(eCvec, 2 * v + 1, x, t, par1, subName);
        eCU = eCL + GetdBridgeSame(eCvec, 2 * v + 2, x, t, par1, subName);
        eCindex_computed += 2;
      }
          ...
          eCL == eCU)  /// ...then we have lost precision before reaching Fmkj.
      {
        ...
        if (eAL[n] < 0.0 || eCL < 0.0) {
          cerr << "Numerical error detected: (m,k) = " << m << "," << k
               ...
               << ". Resorting to G1984-style approximation (s,t) = (" << s
               << "," << t << ") ..." << endl;
          return DrawBridgePMFSameMutationApprox(x, s, t, par1, subName, gen);
        }
    ...
    while (!decision_on_mk_made)  /// Need to refine upper and lower bounds
    {
      ...
      for (int i = 0; i <= n; ++i) {
        ...
        if (2 * (v_used[i] + 1) > eCindex_computed) {
          assert(2 * v_used[i] == eCindex_computed);
          eCL = eCU - GetdBridgeSame(eCvec, 2 * v_used[i] + 1, x, t, par1, subName);
          eCU = eCL + GetdBridgeSame(eCvec, 2 * v_used[i] + 2, x, t, par1, subName);
          eCindex_computed += 2;
        }
               ...
               /// below threshold
{
  ...
  vector<double100> eCvec, eAL, eAU, dL, dU, currsumStore;
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(GetdBridgeSame(eCvec, 0, x, t, par1, subName)), qApprox = 0.0,
            runningMax = 1.0e-300;
  vector<int> v_used;
  map<vector<int>, double100> Amkj;
  int n = -1, Fmkj = 0, eCindex_computed = 0,
...
vector<int> WrightFisher::DrawBridgePMFSameMutationApprox(
    double100 x, double100 s, double100 t, double par1, char subName,
    boost::random::mt19937 &
...
vector<int> WrightFisher::DrawBridgePMFDifferentMutation(
    double100 s, double100 t, double100 x, double par1, char subName, const Options &o,
    boost::random::mt19937 &
              ...
              eCU[n])  /// ...then we have lost precision before reaching Fmk.
      {
        ...
        if (eAL[n] < 0.0 || eCL[n] < 0.0) {
          cerr << "Numerical error detected: (m,k) = " << m << "," << k
               ...
               << ". Resorting to G1984-style approximation (s,t) = (" << s
               << "," << t << ") ..." << endl;
          return DrawBridgePMFDifferentMutationApprox(s, t, x, par1, subName, o, gen);
        }
               ...
               /// below threshold
{
  ...
  double100 eC =
      (t <= o.g1984threshold ? DiscretisedNormCDF(0, t) : QmApprox(0, t, par1, subName, o)) /
      boost::math::beta<double100>(static_cast<double100>(thetaP[0]),
                                   static_cast<double100>(thetaP[1]));

  /// Compute a guess to the mode over (m,k)
  vector<int> modeGuess = mkModeFinder(x, 1.0 - x, s, t, par1, subName, o);
  int mMode = modeGuess[0],
  ...
  double100 currsum = 0.0,
            threshold =
                exp(mkModeFinder_Evaluator(mMode, kMode, x, 1.0 - x, s, t, par1, subName, o) -
                    constContr) *
                1.0e-4,
            runningMax = -1.0e100;
  ...
  while (!mSwitch) {
    ...
    while (!kSwitch) {
      double100 currsum_inc =
          log(QmApprox(m, s, par1, subName, o)) + log(QmApprox(k, t - s, par1, subName, o)) +
          boost::math::lgamma(static_cast<double100>(theta + k)) +
          boost::math::lgamma(static_cast<double100>(theta + m)) -
          boost::math::lgamma(static_cast<double100>(theta + m + k)) -
          constContr;
               ...
               /// large enough
{
  ...
  vector<double100> eCvec, eAL, eAU, eBL, eBU, dL, dU;
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(GetdBridgeInterior(eCvec, 0, x, z, t, par1, subName));
  vector<int> v_used;
  double100 Amkj;
  int n = -1, Fmkj = 0, eCindex_computed = 0;
  ...
  while (!mkj_found) {
    ...
    while (2 * v < Fmkj || eAU[n] > 1.0 || eBU[n] > 1.0) {
      ...
      if (2 * v + 2 > eCindex_computed) {
        assert(2 * v == eCindex_computed);
        eCL = eCU - GetdBridgeInterior(eCvec, 2 * v + 1, x, z, t, par1, subName);
        eCU = eCL + GetdBridgeInterior(eCvec, 2 * v + 2, x, z, t, par1, subName);
        eCindex_computed += 2;
      }
          ...
          eCL == eCU)  /// ...then we have lost precision before reaching Fmkj.
      {
        ...
        if (eAL[n] < 0.0 || eCL < 0.0) {
          cerr << "Numerical error detected: (m,k) = " << m << "," << k
               ...
               << ". Resorting to G1984-style approximation (s,t) = (" << s
               << "," << t << ") ..." << endl;
          return DrawBridgePMFInteriorMutationApprox(x, z, s, t, par1, subName, o, gen);
        }
    ...
    for (int j = 0; j <= k && !mkj_found; ++j) {
      ...
      while (!decision_on_mkj_made)  /// Refine upper and lower bounds
      {
        ...
        for (int i = 0; i <= n; ++i) {
          ...
          if (2 * v_used[i] + 2 > eCindex_computed) {
            assert(2 * v_used[i] == eCindex_computed);
            eCL = eCU - GetdBridgeInterior(eCvec, 2 * v_used[i] + 1, x, z, t, par1, subName);
            eCU = eCL + GetdBridgeInterior(eCvec, 2 * v_used[i] + 2, x, z, t, par1, subName);
            eCindex_computed += 2;
          }
               ...
               /// below the threshold
{
  ...
  vector<double100> eCvec, eAL, eAU, dL, dU, currsumStore;
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(GetdBridgeInterior(eCvec, 0, x, z, t, par1, subName)), qApprox = 0.0;
  vector<int> v_used;
               ...
               /// below the threshold
{
  ...
  while (max(eCOldInc, eCInc) > 0.0 || (!(eC > 0.0))) {
    ...
    eCInc = exp(log(QmApprox(d, t, par1, subName, o)) + zcont -
  ...
  vector<int> modeGuess = mkjModeFinder(
      x, z, s, t, par1, subName, o);  /// Get a guess to location of mode over (m,k,j)
               ...
               /// interior of (0,1) with both time increments large enough
{
  ...
  vector<double100> eCvec, eAL, eAU, eBL, eBU, dL, dU;
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(Getd(eCvec, 0, x, z, t, par1, subName));
  vector<int> v_used;
  double100 Amklj;
  ...
  while (!mklj_found) {
    ...
    while (2 * v < Fmklj || eAU[n] > 1.0 || eBU[n] > 1.0) {
      ...
      if (2 * v + 2 > eCindex_computed) {
        assert(2 * v == eCindex_computed);
        eCL = eCU - Getd(eCvec, 2 * v + 1, x, z, t, par1, subName);
        eCU = eCL + Getd(eCvec, 2 * v + 2, x, z, t, par1, subName);
        eCindex_computed += 2;
      }
          ...
          eCL == eCU)  /// ...then we have lost precision before reaching Fmklj
      {
        ...
        if (eAL[n] < 0.0 || eCL < 0.0) {
          cerr << "Numerical error detected: (m,k) = " << m << "," << k
               ...
               << ". Resorting to G1984-style approximation (s,t) = (" << s
               << "," << t << ") ..." << endl;
          return DrawBridgePMFG1984(x, z, s, t, par1, subName, o, gen);
        }
      ...
      for (int j = 0; j <= k && !mklj_found; ++j) {
        ...
        while (!decision_on_mklj_made)  /// Refine upper and lower bounds
        {
          ...
          for (int i = 0; i <= n; ++i) {
            ...
            if (2 * v_used[i] + 2 > eCindex_computed) {
              assert(2 * v_used[i] == eCindex_computed);
              eCL = eCU - Getd(eCvec, 2 * v_used[i] + 1, x, z, t, par1, subName);
              eCU = eCL + Getd(eCvec, 2 * v_used[i] + 2, x, z, t, par1, subName);
              eCindex_computed += 2;
            }
                                  ...
                                  /// one time increment is below the threshold
{
  ...
  vector<double100> eCvec, eAL, eAU, dL, dU, currsumStore;
  double100 currsumU = 0.0, currsumL = 0.0, eCL = 0.0,
            eCU(Getd(eCvec, 0, x, z, t, par1, subName)), qApprox = 0.0, runningMax = 1.0e-300;
  vector<int> v_used;
  double100 Amklj;
  int n = -1, Fmklj = 0, eCindex_computed = 0;
  /// Mechanism to check whether we have run out of precision due to Gaussian
               ...
               /// threshold
{
  ...
  while (max(eCOldInc, eCInc) > 0.0 || (!(eC > 0.0))) {
    ...
    eCInc = QmApprox(d, t, par1, subName, o) * addon;
    eC += eCInc;
  ...
  vector<int> modeGuess =
      mkljModeFinder(x, z, s, t, par1, subName, o);  /// Get a guess on mode over (m,k,l,j)
  int mMode = modeGuess[0], kMode = modeGuess[1], lMode = modeGuess[2],
      jMode = modeGuess[3];
...
double100 WrightFisher::mkModeFinder_Evaluator(
    int m, int k, double100 x, double100 z, double100 s, double100 t, double par1, char subName,
    const Options &o)  /// Evaluation function for finding mode over (m,k)
{
  assert((m >= 0) && (k >= 0) && (x >= 0.0) && (x <= 1.0) && (z >= 0.0) &&
         (z <= 1.0) && (s > 0.0) && (s < t));
  double100 qm = QmApprox(m, s, par1, subName, o), qk = QmApprox(k, t - s, par1, subName, o);
...
double100 WrightFisher::mkjModeFinder_Evaluator(
    int m, int k, int j, double100 x, double100 z, double100 s, double100 t, double par1, char subName,
...
double100 WrightFisher::mkljModeFinder_Evaluator(
    int m, int k, int l, int j, double100 x, double100 z, double100 s, double100 t, double par1, char subName,
...
double100 WrightFisher::mklModeFinder_Evaluator(
    int m, int k, int l, double100 x, double100 z, double100 s, double100 t, double par1, char subName,
...
vector<int> WrightFisher::mkModeFinder(
    double100 x, double100 z, double100 s, double100 t, double par1, char subName,
    const Options &o)  /// Routine for finding mode over (m,k)
{
  ...
  double100 currMode_eval = mkModeFinder_Evaluator(m, k, x, z, s, t, par1, subName, o);
  ...
  while (!stop) {
    if (currMode_eval < mkModeFinder_Evaluator(m + 1, k, x, z, s, t, par1, subName, o)) {
      m_ud = 1;
    } else if ((m - 1 >= 0) && currMode_eval < mkModeFinder_Evaluator(
                                                   m - 1, k, x, z, s, t, par1, subName, o)) {
      m_ud = -1;
    } else {
      m_ud = 0;
    }
    ...
    m += m_ud;
    currMode_eval = mkModeFinder_Evaluator(m, k, x, z, s, t, par1, subName, o);

    if (currMode_eval < mkModeFinder_Evaluator(m, k + 1, x, z, s, t, par1, subName, o)) {
      k_ud = 1;
    } else if ((k - 1 >= 0) && currMode_eval < mkModeFinder_Evaluator(
                                                   m, k - 1, x, z, s, t, par1, subName, o)) {
      k_ud = -1;
    } else {
      k_ud = 0;
    }
    ...
    const Options &o)  /// Routine for finding mode over (m,k,j)
{
  ...
  double100 currMode_eval = mkjModeFinder_Evaluator(m, k, j, x, z, s, t, par1, subName, o);
  ...
  /// j <= k at all times
  while (!stop) {
    if (currMode_eval < mkjModeFinder_Evaluator(m + 1, k, j, x, z, s, t, par1, subName, o)) {
      m_ud = 1;
    } else if ((m - 1 >= 0) &&
               currMode_eval <
                   mkjModeFinder_Evaluator(m - 1, k, j, x, z, s, t, par1, subName, o)) {
      m_ud = -1;
    } else {
      m_ud = 0;
    }
    ...
    m += m_ud;
    currMode_eval = mkjModeFinder_Evaluator(m, k, j, x, z, s, t, par1, subName, o);

    if (currMode_eval < mkjModeFinder_Evaluator(m, k + 1, j, x, z, s, t, par1, subName, o)) {
      k_ud = 1;
      if (currMode_eval <
          mkjModeFinder_Evaluator(m, k + 1, j + 1, x, z, s, t, par1, subName, o)) {
        j_ud = 1;
      } else if ((j - 1 >= 0) &&
                 (currMode_eval <
                  mkjModeFinder_Evaluator(m, k + 1, j - 1, x, z, s, t, par1, subName, o))) {
        ...
        j_ud = 0;
               ...
               currMode_eval <
                   mkjModeFinder_Evaluator(m, k - 1, j, x, z, s, t, par1, subName, o)) {
      k_ud = -1;
      if ((j + 1 <= k - 1) &&
          (currMode_eval <
           mkjModeFinder_Evaluator(m, k - 1, j + 1, x, z, s, t, par1, subName, o))) {
      ...
      if (k == 0) {
        ...
        if ((j + 1 <= k) && (currMode_eval < mkjModeFinder_Evaluator(
                                                 m, k, j + 1, x, z, s, t, par1, subName, o))) {
          j_ud = 1;
        } else if ((j - 1 >= 0) &&
                   (currMode_eval <
                    mkjModeFinder_Evaluator(m, k, j - 1, x, z, s, t, par1, subName, o))) {
          j_ud = -1;
        } else {
          j_ud = 0;
        }
    ...
    const Options &o)  /// Routine for finding mode over (m,k,l,j)
{
  ...
  double100 currMode_eval = mkljModeFinder_Evaluator(m, k, l, j, x, z, s, t, par1, subName, o);
  ...
  while (!stop) {
    if (currMode_eval <
        mkljModeFinder_Evaluator(m + 1, k, l, j, x, z, s, t, par1, subName, o)) {
      m_ud = 1;
      if (currMode_eval <
          mkljModeFinder_Evaluator(m + 1, k, l + 1, j, x, z, s, t, par1, subName, o)) {
        l_ud = 1;
      } else if ((l - 1 >= 0) &&
      ...
      if (m == 0) {
        ...
        if ((l + 1 <= m) &&
            (currMode_eval <
             mkljModeFinder_Evaluator(m, k, l + 1, j, x, z, s, t, par1, subName, o))) {
          l_ud = 1;
        } else if ((l - 1 >= 0) &&
                   (currMode_eval <
                    mkljModeFinder_Evaluator(m, k, l - 1, j, x, z, s, t, par1, subName, o))) {
    ...
    m += m_ud;
    l += l_ud;
    currMode_eval = mkljModeFinder_Evaluator(m, k, l, j, x, z, s, t, par1, subName, o);

    if (currMode_eval <
        mkljModeFinder_Evaluator(m, k + 1, l, j, x, z, s, t, par1, subName, o)) {
      k_ud = 1;
      if (currMode_eval <
          mkljModeFinder_Evaluator(m, k + 1, l, j + 1, x, z, s, t, par1, subName, o)) {
        j_ud = 1;
      } else if ((j - 1 >= 0) &&
      ...
      if (k == 0) {
        ...
        if ((j + 1 <= k) &&
            (currMode_eval <
             mkljModeFinder_Evaluator(m, k, l, j + 1, x, z, s, t, par1, subName, o))) {
          j_ud = 1;
        } else if ((j - 1 >= 0) &&
                   (currMode_eval <
                    mkljModeFinder_Evaluator(m, k, l, j - 1, x, z, s, t, par1, subName, o))) {
    ...
    const Options &o)  /// Routine for finding mode over (m,k,l)
{
  ...
  double100 currMode_eval = mklModeFinder_Evaluator(m, k, l, x, z, s, t, par1, subName, o);
  ...
  Rerun:
    ...
    if (currMode_eval < mklModeFinder_Evaluator(m, k + 1, l, x, z, s, t, par1, subName, o)) {
      k_ud = 1;
    } else if ((k - 1 >= mkLower) &&
               currMode_eval <
                   mklModeFinder_Evaluator(m, k - 1, l, x, z, s, t, par1, subName, o)) {
      k_ud = -1;
    } else {
      k_ud = 0;
    }
    ...
    k += k_ud;
    currMode_eval = mklModeFinder_Evaluator(m, k, l, x, z, s, t, par1, subName, o);
    int lLower = (thetaP.empty() || (!(thetaP[0] > 0.0) && !(z > 0.0)) ? 1 : 0),
        lUpper =
            (thetaP.empty() || (!(thetaP[1] > 0.0) && !(z < 1.0)) ? m - 1 : m);

    if (currMode_eval < mklModeFinder_Evaluator(m + 1, k, l, x, z, s, t, par1, subName, o)) {
      m_ud = 1;
      if (currMode_eval <
          mklModeFinder_Evaluator(m + 1, k, l + 1, x, z, s, t, par1, subName, o)) {
        l_ud = 1;
      } else if ((l - 1 >= lLower) &&
                 (currMode_eval <
                  mklModeFinder_Evaluator(m + 1, k, l - 1, x, z, s, t, par1, subName, o))) {
        ...
        l_ud = 0;
               ...
               currMode_eval <
                   mklModeFinder_Evaluator(m - 1, k, l, x, z, s, t, par1, subName, o)) {
      m_ud = -1;
      if ((l + 1 <= lUpper - 1) &&
          (currMode_eval <
           mklModeFinder_Evaluator(m - 1, k, l + 1, x, z, s, t, par1, subName, o))) {
      ...
      if (m == mkLower) {
        ...
        if ((l + 1 <= lUpper) &&
            (currMode_eval <
             mklModeFinder_Evaluator(m, k, l + 1, x, z, s, t, par1, subName, o))) {
          l_ud = 1;
        } else if ((l - 1 >= lLower) &&
                   (currMode_eval <
                    mklModeFinder_Evaluator(m, k, l - 1, x, z, s, t, par1, subName, o))) {
...
pair<double100, int> WrightFisher::DrawBridgepoint(
    double100 x, double100 z, double100 t1, double100 t2, double100 s, double par1, char subName,
    const Options &o,
    boost::random::mt19937
               ...
               /// (t1,t2)
{
       ...
       260.0))  /// Use diffusion approximations
  {
    ...
    double100 y1 =
        DrawEndpoint(x, t1, s, par1, subName, o, gen)
  ...
  } else  /// Else use bridge simulator
  {
    if (!(x > 0.0))  /// x = 0
    {
      if (!(z > 0.0))  /// x = 0 & z = 0
      {
        ...
        {
          mklj = DrawBridgePMFSameMutationApprox(x, s - t1, t2 - t1, par1, subName, gen);
          ...
          mklj =
              DrawBridgePMFSameMutationOneQApprox(x, s - t1, t2 - t1, par1, subName, o, gen);
        } else  /// Time increments are large enough for alternating series
                /// method
        {
          mklj = DrawBridgePMFSameMutation(x, s - t1, t2 - t1, par1, subName, o, gen);
        }
      ...
      } else if (!(z < 1.0))  /// x = 0 & z = 1
      {
        ...
        {
          mklj =
              DrawBridgePMFDifferentMutationApprox(s - t1, t2 - t1, x, par1, subName, o, gen);
        }  /// One time increment both below threshold
        else if ((s - t1 <= o.g1984threshold && t2 - s > o.g1984threshold) ||
                 (s - t1 > o.g1984threshold && t2 - s <= o.g1984threshold)) {
          mklj = DrawBridgePMFDifferentMutationOneQApprox(s - t1, t2 - t1, x, par1, subName, o,
                                                          gen);
        } else  /// Time increments are large enough for alternating series
                /// method
        {
          mklj = DrawBridgePMFDifferentMutation(s - t1, t2 - t1, x, par1, subName, o, gen);
        }
      ...
      } else  /// x = 0 & z in (0,1)
      {
        if (s - t1 <= o.g1984threshold &&
            t2 - s <=
                o.g1984threshold)  /// Time increments both below threshold
        {
          mklj = DrawBridgePMFInteriorMutationApprox(x, z, s - t1, t2 - t1, par1, subName, o,
                                                     gen);
        }  /// One time increment both below threshold
        else if ((s - t1 <= o.g1984threshold && t2 - s > o.g1984threshold) ||
                 (s - t1 > o.g1984threshold && t2 - s <= o.g1984threshold)) {
          mklj = DrawBridgePMFInteriorMutationOneQApprox(x, z, s - t1, t2 - t1, par1, subName,
                                                         o, gen);
        } else  /// Time increments are large enough for alternating series
                /// method
        {
          mklj = DrawBridgePMFInteriorMutation(x, z, s - t1, t2 - t1, par1, subName, o, gen);
        }
    ...
    } else  /// x in (0,1)
    {
      ...
      } else  /// x in (0,1) & z in (0,1)
      {
        ...
        {
          mklj = DrawBridgePMFG1984(x, z, s - t1, t2 - t1, par1, subName, o, gen);
        }  /// One time increment both below threshold
        else if ((s - t1 <= o.g1984threshold && t2 - s > o.g1984threshold) ||
                 (s - t1 > o.g1984threshold && t2 - s <= o.g1984threshold)) {
          mklj = DrawBridgePMFOneQApprox(x, z, s - t1, t2 - t1, par1, subName, o, gen);
        } else  /// Time increments are large enough for alternating series
                /// method
        {
          mklj = DrawBridgePMF(x, z, s - t1, t2 - t1, par1, subName, o, gen);
        }
      ...
      if (!(x > 0.0)) {
        ...
        if (!(z > 0.0)) {
          ...
          if (s - t1 <= o.g1984threshold && t2 - s <= o.g1984threshold) {
            mklj =
                DrawBridgePMFUnconditionalApprox(x, z, s - t1, t2 - t1, par1, subName, o, gen);
          } else if ((s - t1 <= o.g1984threshold &&
                      t2 - s > o.g1984threshold) ||
                     (s - t1 > o.g1984threshold &&
                      t2 - s <= o.g1984threshold)) {
            mklj = DrawBridgePMFUnconditionalOneQApprox(x, z, s - t1, t2 - t1, par1, subName,
                                                        o, gen);
          } else {
            mklj = DrawBridgePMFUnconditional(x, z, s - t1, t2 - t1, par1, subName, o, gen);
          }
        ...
        {
          ThetaResetter();
          return DrawBridgepoint(x, z, t1, t2, s, par1, subName, o, gen);
        }
      }
    } else if (!(thetaP[0] > 0.0) || !(thetaP[1] > 0.0)) {
      if (!(thetaP[0] > 0.0)) {
        if (!(x > 0.0)) {
          ...
          if (!(z > 0.0)) {
            ...
            if (s - t1 <= o.g1984threshold && t2 - s <= o.g1984threshold) {
              mklj = DrawBridgePMFUnconditionalApprox(x, z, s - t1, t2 - t1, par1, subName, o,
                                                      gen);
            } else if ((s - t1 <= o.g1984threshold &&
                        t2 - s > o.g1984threshold) ||
                       (s - t1 > o.g1984threshold &&
...
vector<vector<double100>> WrightFisher::NonNeutralDrawBridge(
    double100 x, double100 t1, double100 t2, double100 z, double par1, char subName, bool Absorption,
    const Options &o,
    boost::random::mt19937
    ...
    } else  /// kappa > 0 - generate skeleton points and check them
    {
      for (vector<double100>::iterator itt = times.begin(), itm = marks.begin();
        ...
        if (itt == times.begin()) {
          if (Absorption) {
            path.push_back(
                DrawUnconditionedBridge(x, z, t1, t2, *itt, par1, subName, o, gen).first);
          } else {
            path.push_back(
                DrawBridgepoint(x, z, t1, t2, *itt, par1, subName, o, gen)
                    .first);  /// Generate skeleton points sequentially
          }
                                  ...
                                  /// we are not at the last one yet
        {
          if (Absorption) {
            path.push_back(DrawUnconditionedBridge(path.back(), z, *(itt - 1),
                                                   t2, *itt, par1, subName, o, gen)
                               .first);
          } else {
            path.push_back(
                DrawBridgepoint(path.back(), z, *(itt - 1), t2, *itt, par1, subName, o, gen)
                    .first);  /// Generate skeleton points sequentially
          }
        ...
        } else  /// We are at the last skeleton point
        {
          if (Absorption) {
            path.push_back(
                DrawUnconditionedBridge(path.back(), z, *(itt - 1), t2, *itt, par1, subName, o,
                                        gen)
...
pair<double100, int> WrightFisher::NonNeutralDrawBridgepoint(
    double100 x, double100 t1, double100 t2, double100 z, double100 testT, double par1, char subName,
               ...
               /// at time testT
{
  ...
  vector<vector<double100>> skeleton = NonNeutralDrawBridge(
      x, t1, t2, z, Absorption, par1, subName, o, gen);  /// Create skeleton points for bridge
  ...
  if (Absorption) {
    return DrawUnconditionedBridge(*xIt, *(xIt + 1), *timeIt, *(timeIt + 1),
                                   testT, par1, subName, o, gen);
  } else {
    return DrawBridgepoint(*xIt, *(xIt + 1), *timeIt, *(timeIt + 1), testT, par1, subName, o,
...
void WrightFisher::DiffusionRunner(
    int nSim, double100 x, double100 startT, double100 endT, double par1, char subName, bool Absorption,
                                 ...
                                 /// diffusion draws
{
  ...
  while (nosamples < nSim + 1) {/// nSim loop -- different cases call different functions
    if (non_neutral) {
      saveFile
          << NonNeutralDrawEndpoint(x, startT, endT, Absorption, par1, subName, o, gen).first
          << "\n";
    } else {
      if (Absorption) {
        saveFile << DrawUnconditionedDiffusion(x, endT - startT, par1, subName, o, gen).first
                 << "\n";
      } else {
        saveFile << DrawEndpoint(x, startT, endT, par1, subName, o, gen).first << "\n"; /// most common call
      }
    }
...
void WrightFisher::BridgeDiffusionRunner(
    ...
    double100 sampleT, double par1, char subName, bool Absorption, string &Filename,
                                 ...
                                 /// bridge draws
{
  ...
  while (nosamples < nSim + 1) {
    if (non_neutral) {
      saveFile << NonNeutralDrawBridgepoint(x, startT, endT, z, sampleT, par1, subName,
                                            Absorption, o, gen)
                      .first
               << "\n";
    } else {
      if (Absorption) {
        saveFile << DrawUnconditionedBridge(x, z, startT, endT, sampleT, par1, subName, o, gen)
                        .first
                 << "\n";
      } else {
        ThetaResetter();
        saveFile << DrawBridgepoint(x, z, startT, endT, sampleT, par1, subName, o, gen).first
                 << "\n";
      }
    }
...
void WrightFisher::DiffusionDensityCalculator(
    int meshSize, double100 x, double100 startT, double100 endT, double par1, char subName,
  ...
  if (non_neutral) {
    ...
    while (counter <= meshSize) {
      ...
      if (Absorption) {
        if ((x > 0.0) && (x < 1.0)) {
          saveFile << y << " "
                   << UnconditionedDiffusionDensity(x, y, timeInc, par1, subName, o) << "\n";
        } else {
          if (!(x > y) && !(x < y)) {
            saveFile << y << " " << 1.0 << "\n";
          } else {
        ...
        saveFile << y << " " << DiffusionDensityApproximation(x, y, timeInc, par1, subName, o)
                 << "\n";
      }
                                 ...
                                 /// bridge transition density
{
  ...
  while (counter <= meshSize) {
    ...
    if (non_neutral) {
      ...
      if (Absorption) {
        if ((x > 0.0) && (x < 1.0)) {
          saveFile << y << " "
                   << UnconditionedBridgeDensity(x, z, y, sampleInc, timeInc, par1, subName, o)
                   << "\n";
        } else {
          if (!(x > y) && !(x < y)) {
            saveFile << y << " " << 1.0 << "\n";
        ...
        saveFile << y << " " << BridgeDensity(x, z, y, sampleInc, timeInc, par1, subName, o)
                 << "\n";
      }
    }
  ...
  saveFile.close();
    ...
    void WrightFisher::DualRunner(
        int nSim, double100 startT, double100 endT, double par1, char subName,
        string& Filename, double100 diffusion_threshold,
        double100 bridge_threshold)  /// Function to generate specified number of
        /// diffusion draws
    {
        ...
        int nosamples = 1, loader = floor(0.1 * nSim), loader_count = 1;
        while (nosamples < nSim + 1) {
            saveFile << WrightFisher::DrawAncestralProcess(endT - startT, par1, subName, o, gen).first << "\n";
            nosamples++;
            if (nosamples % (loader * loader_count) == 0) {
                std::cout << "Simulated " << nosamples << " samples." << endl;
                loader_count++;
        ...
        std::cout << "Dual simulation complete." << endl;
        saveFile.close();
    }


    void WrightFisher::DiffusionTraj(vector<double100> times, double par1, char subName,
        string& Filename, double100 diffusion_threshold,
        double100 bridge_threshold)  /// Function to generate specified number of
        /// diffusion draws
    {
        ...
        double100 x_curr; double100 prevT; double100 currT;  vector<double100> x(n_iter);
        x_curr = 0.5; prevT = 0;
        for (int i = 0; i < n_iter + 1; i++) {
            currT = times[i];
            x[i] = WrightFisher::DrawEndpoint(x_curr, prevT, currT, par1, subName, o, gen).first;
            saveFile << x[i] <<",";
            /// next
            x_curr = x[i]; prevT = currT;
        }

        std::cout << "Diffusion trajectory simulation complete." << endl;
        saveFile.close();
    }
